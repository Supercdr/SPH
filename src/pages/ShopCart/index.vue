      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="cart.isChecked==1" @change="changeIsChecked(cart)"/>
          </li>
...
export default {
  ...
  computed: {
    ...
    checkedNum(){
      return this.cartInfoList.reduce((num,item)=>num+item.isChecked,0)
    },
    checkedSum(){
      let sum=0
  ...
  methods: {
    ...
    async changeIsChecked(cart){
      try{
        await this.$store.dispatch('changeCheckCart',{skuId:cart.skuId,isChecked:cart.isChecked==1?0:1})
        this.getData()
      }catch(error){
        alert(error.message)
      }
